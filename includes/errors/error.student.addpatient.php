<?php
// $studentChecker = new StudentControl();
$path = '../pages/addpatient.php';
...
if(empty($firstname) && empty($lastname) && empty($email) && empty($pwd) && empty($rptpwd) && empty($birthday) && empty($weight) && 
   ...
   empty($emergency_phone) && empty($studentnumber)){
        ...
        $_SESSION['weight'] = $weight; $_SESSION['height'] = $height;
...
if($studentControl->studentnumberExists($conn, $studentnumber) !== false){
    ...
    $_SESSION['error'] = 'studentnumberalreadyexist';
    header("Location: $path");
    exit();
}
...
if (!preg_match("/^[0-9]+-[0-9]+-[a-zA-z]+-[0-9]+$/", $studentnumber)){
    ...
    $_SESSION['error'] = 'invalidstudentnumber';
    header("Location: $path");
    exit();
}

...
if(isset($_POST['patientid'])){
    ...
    // PHYSICAL ROW
    $phyQuery = "SELECT * FROM physical_examination WHERE ".$patient."id=$id ";
    $phyResult = mysqli_query($conn, $phyQuery);
    $physicalRow = mysqli_fetch_array($phyResult);

    // MEDICAL HISTORY ROW
    $medQuery = "SELECT * FROM medical_history WHERE ".$patient."id=$id ";
    $medResult = mysqli_query($conn, $medQuery);
    $medRow = mysqli_fetch_array($medResult);

    // ALLERGY ROW
    $allQuery = "SELECT * FROM allergy WHERE ".$patient."id=$id ";
    ...
    <div class="container fetchprofile p-0">
        ...
        <div class="bg-light info-card  p-3">
            ...
            <div class="btn-footer bg-light pt-1 pb-3 text-end pe-4">
                <a href="../pages/request.search.php" class="btn bg-secondary me-2 text-white ">Cancel</a>
                <button type="button" class="btn text-white bg-green proceedBtn" id="<?php echo $id; ?>">Proceed</button>
            </div>
    </div>
